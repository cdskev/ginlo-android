...
public class DispatchQueuePool {
    ...
    private final static String TAG = "DispatchQueuePool";
    ...
    public DispatchQueuePool(int count) {
        maxCount = count;
        guid = SecurityUtil.randomNextInt();
    }
    ...
    public void execute(Runnable runnable) {
        ...
        if (!busyQueues.isEmpty() && (totalTasksCount / 2 <= busyQueues.size() || queues.isEmpty() && createdCount >= maxCount)) {
            ...
            queue = new DispatchQueue("DispatchQueuePool" + guid + "_" + SecurityUtil.randomNextInt());
