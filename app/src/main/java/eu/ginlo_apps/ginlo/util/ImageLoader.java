...
public abstract class ImageLoader
        implements OnImageDataChangedListener {
    private static final int FADE_IN_TIME = 200;

    private final Object mPauseWorkLock = new Object();
    private final int mImageSize;
    private final Resources mResources;
    private final boolean mUseThreadExecutor;
    private eu.ginlo_apps.ginlo.util.ImageCache mImageCache;
    ...
    protected ImageLoader(Context context,
                          int imageSize,
                          boolean useThreadExecutor) {
        mResources = context.getResources();
        mImageSize = imageSize;
        mUseThreadExecutor = useThreadExecutor;
    }

    /**
     * Returns true if the current work has been canceled or if there was no work
     * in progress on this image view. Returns false if the work in progress
     * deals with the same data. The work is not stopped in that case.
     */
    private static boolean cancelPotentialWork(Object data,
                                               ImageView imageView) {
     ...
     * width and height
     */
    static Bitmap decodeSampledBitmapFromDescriptor(FileDescriptor fileDescriptor,
                                                    int reqWidth,
                                                    int reqHeight) {
     ...
     * object when decoding bitmaps using the decode* methods from {@link
     * BitmapFactory}. This implementation calculates the closest inSampleSize
     * that will result in the final decoded bitmap having a width and height
     * equal to or larger than the requested width and height. This
    ...
    public void loadImage(Object data,
        ...
        if (bitmap != null) {
            ...
            processBitmapFinished(data, imageView);
        } else if (cancelPotentialWork(data, imageView)) {
            final BitmapWorkerTask task = new BitmapWorkerTask(imageView);
            final AsyncDrawable asyncDrawable = new AsyncDrawable(mResources, loadingBitmap, task);

            imageView.setImageDrawable(asyncDrawable);
            if (mUseThreadExecutor) {
                try {
                    task.executeOnExecutor(BitmapWorkerTask.THREAD_POOL_EXECUTOR, data);
    ...
    /**
     * Adds an {@link eu.ginlo_apps.ginlo.util.ImageCache} to this image loader.
     *
     * @param fragmentManager     A FragmentManager to use to retain the cache
     *                            over configuration changes such as an
     *                            orientation change.
    ...
    /**
     * Subclasses should override this for Notifying that the image load process
     * is finished with SUCCESS. This will be executed in the caller(main)
     * thread.
     *
     * @param data      The data to identify which image to process, as provided by
     *                  {@link ImageLoader#loadImage(Object, ImageView)}
     * @param imageView
     */
    protected abstract void processBitmapFinished(Object data, ImageView imageView);
    ...
    private void setImageBitmap(ImageView imageView,
        ...
        if (mFadeInBitmap) {
            ...
            if (imageView instanceof MaskImageView) {
                roundBitmap = ((MaskImageView) imageView).getMaskImageFromImage(bitmap);
            }
     ...
     * ListView or GridView is being scrolled using a {@link
     * android.widget.AbsListView.OnScrollListener} to keep scrolling smooth.
     *
     * <p>If work is paused, be sure setPauseWork(false) is called again before
     * your fragment or activity is destroyed (for example during {android.app.Activity#onPause()}), or there is a risk the background thread
     * will never finish.
     */
    ...
    /**
     * Callback from ChatImageController
     */
    @Override
    public void onImageDataChanged(String guid) {
        if (mImageCache != null) {
            mImageCache.removeBitmapFromCache(guid);
        }
    }

    /**
     * A custom Drawable that will be attached to the imageView while the work is
     * in progress. Contains a reference to the actual worker task, so that it
     * can be stopped if a new binding is required, and makes sure that only the
     * last started worker process can bind its result, independently of the
     * finish order.
     */
