...
public class IabResult {
    ...
    public IabResult(int response,
                     String message) {
        mResponse = response;
        if ((message == null) || (message.trim().length() == 0)) {
            mMessage = eu.ginlo_apps.ginlo.billing.IabHelper.getResponseDesc(response);
        } else {
            mMessage = message + " (response: " + eu.ginlo_apps.ginlo.billing.IabHelper.getResponseDesc(response) + ")";
        }
    }
