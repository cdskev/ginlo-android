...
import eu.ginlo_apps.ginlo.billing.Base64;
import eu.ginlo_apps.ginlo.billing.Base64DecoderException;
...
class Security {
     ...
     * @param signature       the signature for the data, signed with the
     *                        private key
     */
    ...
    private static PublicKey generatePublicKey(String encodedPublicKey) {
        try {
            byte[] decodedKey = eu.ginlo_apps.ginlo.billing.Base64.decode(encodedPublicKey);
            KeyFactory keyFactory = KeyFactory.getInstance(KEY_FACTORY_ALGORITHM);

            return keyFactory.generatePublic(new X509EncodedKeySpec(decodedKey));
        } catch (NoSuchAlgorithmException e) {
            LogUtil.e(Security.class.getName(), "NO_INIT_ERROR");
