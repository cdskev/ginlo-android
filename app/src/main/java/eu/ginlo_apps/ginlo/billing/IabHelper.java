...
import eu.ginlo_apps.ginlo.billing.Security;
 ...
 * in order to start using the object. To perform setup, call the {@link
 * #startSetup} method and provide a listener; that listener will be notified
 * when setup is complete, after which (and not before) you may call other
 * methods. After setup is complete, you will typically want to request an
...
public class IabHelper {
     ...
     * thread.
     *
     * @param ctx             Your application or Activity context. Needed to
     *                        bind to the in-app billing service.
     * @param base64PublicKey Your application's public key, encoded in base64.
     *                        This is used for verification of purchase
     *                        signatures. You can find your app's base64-encoded
     *                        public key in your application's page on Google
     *                        Play Developer Console. Note that this is NOT your
     ...
     *                 complete.
     * @throws IllegalStateException [!EXC_DESCRIPTION!]
     */
     ...
     * an in-app purchase, which will involve bringing up the Google Play screen.
     * The calling activity will be paused while the user interacts with Google
     * Play, and the result will be delivered via the activity's {@link
     * Activity#onActivityResult} method, at which point you must
     * call this object's {@link #handleActivityResult} method to continue the
     ...
     * @param listener    The listener to notify when the purchase process
     *                    finishes
     * @param extraData   Extra data (developer payload), which will be returned
     *                    with the purchase data when the purchase completes.
     *                    This extra data will be permanently bound to that
     *                    purchase and will always be returned when the purchase
     *                    is queried.
     */
    private void launchPurchaseFlow(Activity act,
                                    String sku,
                                    String itemType,
                                    int requestCode,
                                    OnIabPurchaseFinishedListener listener,
        ...
        try {
            logDebug("Constructing buy intent for " + sku + ", itemType: " + itemType
                    + ", requestCode: " + requestCode + ", extraData: " + extraData);
    ...
    public boolean handleActivityResult(int requestCode,
        ...
        logDebug("Response from purchaseFlow: " + responseCode + ", DATA: " + purchaseData + ", SIGNATURE: " + dataSignature);
        ...
        if ((resultCode == Activity.RESULT_OK) && (responseCode == BILLING_RESPONSE_RESULT_OK)) {
            ...
            eu.ginlo_apps.ginlo.billing.Purchase purchase = null;

            try {
                purchase = new eu.ginlo_apps.ginlo.billing.Purchase(mPurchasingItemType, purchaseData, dataSignature);

                String sku = purchase.getSku();

                // Verify signature
                if (!eu.ginlo_apps.ginlo.billing.Security.verifyPurchase(mSignatureBase64, purchaseData, dataSignature)) {
    ...
    /**
     * @throws IabException [!EXC_DESCRIPTION!]
     */
    ...
    /**
     * Queries the inventory. This will query all owned items from the server, as
     * well as information on additional skus, if specified. This method may
     * block or take long to execute. Do not call from a UI thread. For that, use
     ...
     *                        regardless of ownership. Ignored if null or if
     ...
     */
    private Inventory queryInventory(boolean querySkuDetails,
     ...
     */
    private void consume(eu.ginlo_apps.ginlo.billing.Purchase itemInfo)
            throws IabException {
        checkNotDisposed();
     ...
     *                 finishes.
     */
    public void consumeAsync(eu.ginlo_apps.ginlo.billing.Purchase purchase,
                             OnConsumeFinishedListener listener) {
        checkNotDisposed();
        checkSetupDone("consume");

        List<eu.ginlo_apps.ginlo.billing.Purchase> purchases = new ArrayList<>();
     ...
     *                  finishes.
     */
    public void consumeAsync(List<eu.ginlo_apps.ginlo.billing.Purchase> purchases,
    ...
    /**
     * @throws RuntimeException [!EXC_DESCRIPTION!]
     */
    ...
    private void flagStartAsync(String operation) {
        if (mAsyncInProgress) {
            throw new IllegalStateException("Can't start async operation (" + operation
                    + ") because another async operation(" + mAsyncOperation
                    + ") is in progress.");
        }
     ...
     * @throws RemoteException [!EXC_DESCRIPTION!]
     */
    private int queryPurchases(Inventory inv,
                               String itemType)
        ...
        do {
            ...
            logDebug("PlayStore query returned ownedSKUs: " + ownedSkus);
            logDebug("PlayStore query returned purchaseDataList: " + purchaseDataList);
            logDebug("PlayStore query returned signatureList: " + signatureList);
            ...
            for (int i = 0; i < purchaseDataList.size(); ++i) {
                ...
                if (eu.ginlo_apps.ginlo.billing.Security.verifyPurchase(mSignatureBase64, purchaseData, signature)) {
                    logDebug("Sku is owned: " + sku);

                    eu.ginlo_apps.ginlo.billing.Purchase purchase = new eu.ginlo_apps.ginlo.billing.Purchase(itemType, purchaseData, signature);
    ...
    private void consumeAsyncInternal(final List<eu.ginlo_apps.ginlo.billing.Purchase> purchases,
        ...
        (new Thread(new Runnable() {
            public void run() {
                ...
                for (eu.ginlo_apps.ginlo.billing.Purchase purchase : purchases) {
                    try {
                        consume(purchase);
    ...
    public interface OnIabPurchaseFinishedListener {
        ...
        void onIabPurchaseFinished(IabResult result,
                                   eu.ginlo_apps.ginlo.billing.Purchase info);
    }
    ...
    public interface OnConsumeFinishedListener {
         ...
         */
        void onConsumeFinished(eu.ginlo_apps.ginlo.billing.Purchase purchase,
                               IabResult result);
    }
    ...
    interface OnConsumeMultiFinishedListener {
         ...
         */
        void onConsumeMultiFinished(List<Purchase> purchases,
